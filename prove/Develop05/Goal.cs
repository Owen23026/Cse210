...
{
    return [_title, _description, _count.ToString(), _total.ToString(), _points.ToString()];
}

public void PullInfo(string[] info)
{
    this._title = info[0];
    this._description = info[1];
    this._count = int.Parse(info[2]);
    this._total = int.Parse(info[3]);
    this._points = int.Parse(info[4]);
}
